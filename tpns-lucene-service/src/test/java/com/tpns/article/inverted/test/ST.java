 ...
 * <tt>equals()</tt> or <tt>hashCode()</tt>. The <em>put</em>, <em>contains</em>
...
public class ST<Key extends Comparable<Key>, Value> implements Iterable<Key> {
	 ...
	 * over all of the keys in a symbol table named <tt>st</tt>, use the foreach
	...
	public Key ceil(final Key key) {
		...
		final SortedMap<Key, Value> tail = st.tailMap(key);
		if (tail.isEmpty()) {
	...
	public Key floor(final Key key) {
		...
		final SortedMap<Key, Value> head = st.headMap(key);
		if (head.isEmpty()) {
			throw new NoSuchElementException();
		}
		return head.lastKey();
